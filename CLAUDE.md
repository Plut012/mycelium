...
1. `manifest.ts` — id, name, category, ports, parameters
2. `engine.ts` — extends `ModuleEngine`, creates AudioNodes in `create()`, cleans up in `destroy()`
3. `Module.svelte` — uses shared UI primitives (Knob, Slider, Toggle), accepts engine + theme props
4. Test: module appears in browser, connects, produces/processes signal
...
- CSS: scoped per component, theme values consumed via props not globals
